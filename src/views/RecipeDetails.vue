<template>
  ...
  <div class="text-center">
    <span
      class="bg-purple-300 mr-2 p-1 rounded text-white"
      v-for="category in recipeDetail.categories"
      :key="category.id"
      >{{ category }}</span
    >
  </div>
  ...
  <div v-if="recipeDetail.ingredients.length >= 1">
    <ol
  ...
  <div v-else>Parece que olvidaste los ingredientes ='(</div>
  <div class="mt-4">Pasos</div>
  <div v-if="recipeDetail.steps.length >= 1">
    <ol
      class="list-disc pl-4"
  ...
  <div v-else>Parece que olvidaste los pasos ='(</div>
</template>
